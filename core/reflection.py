...
class OrionReflection:
    ...
    def post_mortem(self, ledger) -> dict:
        ...
        prompt = (
            f"ACTION HISTORY:\n{history_text}\n\n"
            "Analyze this execution history. Did it succeed? Did it fail?\n"
            "CRITICALLY: Did the user express any preference or constraint? (e.g., 'Use click', 'Avoid argparse')\n"
            "Return ONLY a JSON object:\n"
            "{\n"
            "  \"success\": true,\n"
            "  \"lesson\": \"Single sentence lesson learned.\",\n"
            "  \"user_preference\": \"User prefers CLICK over ARGPARSE.\" (or null)\n"
            "}"
        )
        ...
        try:
            ...
            if lesson:
                # Store Lesson
                self.memory.add(f"lesson_{len(recent_actions)}", lesson, mtype="LESSON")
                print(f"[REFLECTION] 🧠 Learned: {lesson}")

            # Store User Preference
            pref = data.get("user_preference")
            if pref:
                 self.memory.add(f"pref_{len(recent_actions)}", pref, mtype="USER_PREF")
