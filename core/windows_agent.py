...
class OrionWindowsAgent:
    ...
    async def connect(self):
        ...
        while self.running and retry_count < max_retries:
            try:
                ...
                retry_count += 1
                print(f"[ORION AGENT] Connection refused ({retry_count}/{max_retries}). Retrying in 5s...")
                await asyncio.sleep(5)
            except Exception as e:
                self.connected = False
                retry_count += 1
                print(f"[ORION AGENT] Connection error: {e} ({retry_count}/{max_retries}). Retrying in 5s...")
                await asyncio.sleep(5)
            finally:
                if self.sock:
                    self.sock.close()
        ...
        if retry_count >= max_retries:
            print("[ORION AGENT] Max retries reached. Shutting down connection attempts.")
            self.running = False
    ...
    async def monitor_events(self):
        ...
        while self.running:
            try:
                ...
                print(f"[ORION AGENT] Monitor events error: {e}") # Log the error
    ...
    async def telemetry_loop(self):
        ...
        try:
            ...
            class AsyncWindowsFileEventHandler(FileSystemEventHandler):
                def __init__(self, agent_instance, base_path):
                    ...
                    self.excluded_paths = [p.lower() for p in self.excluded_paths]

                def _is_excluded(self, path):
                    path_lower = path.lower()

                    # Ignore the agent's own directory regardless of where it is placed
                    agent_dir = os.path.abspath(os.path.dirname(__file__)).lower()
                ...
                def on_created(self, event):
                    if not event.is_directory and not self._is_excluded(event.src_path):
                        self._dispatch_telemetry("FILE_CREATED", event.src_path)

                def on_deleted(self, event):
                    if not event.is_directory and not self._is_excluded(event.src_path):
                        self._dispatch_telemetry("FILE_DELETED", event.src_path)

                def on_modified(self, event):
                    if not event.is_directory and not self._is_excluded(event.src_path):
                        self._dispatch_telemetry("FILE_MODIFIED", event.src_path)

                def on_moved(self, event):
                    if not event.is_directory and not self._is_excluded(event.src_path) and not self._is_excluded(event.dest_path):
                        self._dispatch_telemetry("FILE_MOVED", {"src": event.src_path, "dest": event.dest_path})

            self.observer = Observer()
            for drive in drive_letters:
                if os.path.exists(drive):
                    event_handler = AsyncWindowsFileEventHandler(self, drive)
                    self.observer.schedule(event_handler, drive, recursive=True)
            self.observer.start()
        except ImportError:
        ...
        while self.running:
            ...
            # Idle optimization: drop out deep scans and limit heavy resource pulls if CPU starts spiking wildly

            # 1. Process List tracking
            try:
                # Use executor since psutil can sporadically block locally depending on WMI state
                ...
                print(f"[ORION AGENT] Process tracking error: {e}") # Log the error
