...
start "ORION Backend (Portable)" /D "%USB_ROOT%\core" cmd /k "python server.py"

echo Waiting for API to bind...
ping 127.0.0.1 -n 6 >nul

echo [ORION] Launching Windows Telemetry Agent...
start "ORION Telemetry Agent" /D "%USB_ROOT%\core" cmd /k "python windows_agent.py"

echo [ORION] Launching Frontend Interface...
start "ORION Dashboard" /D "%USB_ROOT%\orion_ui" cmd /k "npm run dev"
