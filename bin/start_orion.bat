...
start "ORION BRAIN & API" wsl -d Ubuntu -u rohith bash -c "cd ~/ORION && source ~/ORION-env/bin/activate && python3 server.py; exec bash"

:: 3. Launch Body (Windows Agent) - New Window

:: 4. Launch Body (Windows Agent) - New Window
start "ORION BODY" cmd /k "cd /d %ORION_DIR% && python windows_agent.py"

:: 5. Launch Frontend - New Window
start "ORION DASHBOARD" wsl -d Ubuntu -u rohith bash -c "cd ~/ORION/orion && npm install && npm run dev; exec bash"
